...
`1.0.10` is treated as already released. Epic 1 validates the corrective path from published artifacts to the `1.0.11` candidate and records downstream ownership. Epic 2 subsequently closed the migration blockers exposed by this harness.
...
- `upgrade-0.10`: all lifecycle, migration, repair, and rollback assertions passed;
- `upgrade-0.11`: all lifecycle, migration, repair, and rollback assertions passed;
- `upgrade-1.0.9`: all lifecycle, migration, repair, and rollback assertions passed;
- `upgrade-1.0.10`: all lifecycle, migration, repair, and rollback assertions passed.

The final artifact matrix run `20260830T195004Z-2367` passed `360/360` steps with no failed or expected-failed steps. Earlier E1 migration failures were retained as reproducible evidence until their E2 fixes passed the combined release gate.
