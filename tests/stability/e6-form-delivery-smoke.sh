...
WORKDIR="${CF7TG_E6_WORKDIR:-$(mktemp -d "${TMPDIR:-/tmp}/cf7tg-e6-delivery-${RUN_ID}.XXXXXX")}"
RESULTS_DIR="${CF7TG_E6_RESULTS_DIR:-${WORKDIR}/results}"
...
WP_VERSION="${CF7TG_E6_WP_VERSION:-$(jq -r '.wordpress_version // .wordpress.default_core_version // "7.1"' <<<"${CURRENT_ROW}")}"
PHP_VERSION="${CF7TG_E6_PHP_VERSION:-$(jq -r '.php_version // "8.3"' <<<"${CURRENT_ROW}")}"
WP_CLI_IMAGE="${CF7TG_E6_WP_CLI_IMAGE:-$(jq -r '.wp_cli_image // .wordpress.default_cli_image // "wordpress:cli-php8.3"' <<<"${CURRENT_ROW}")}"
CF7_VERSION="${CF7TG_E6_CF7_VERSION:-$(jq -r '.contact_form_7_version // .dependencies.contact_form_7.default_version // "6.1.7"' <<<"${CURRENT_ROW}")}"
...
WEB_PORT="${CF7TG_E6_WEB_PORT:-$(shuf -i 20000-45000 -n 1)}"
BASE_URL="${CF7TG_E6_BASE_URL:-http://127.0.0.1:${WEB_PORT}}"
IMAGE_PROBE_TIMEOUT="${CF7TG_E6_IMAGE_PROBE_TIMEOUT:-90s}"
INSTALL_BROWSER="${CF7TG_E6_INSTALL_BROWSER:-1}"
KEEP_WORKDIR=0
FAILURES=0
SUMMARY_WRITTEN=0
CURRENT_PROJECT=""
...
REQUIRED_CHECKS='["fake-transport-active","public-form-renders","cf7-submit-success","send-message-attempts","no-unexpected-recipient","no-token-leakage","no-page-errors","no-console-errors","partial-failure-cf7-success","partial-failure-recipient-continuity","partial-failure-evidence-redacted","partial-failure-no-page-errors","partial-failure-no-console-errors","admin-empty-state","admin-bot-created","admin-bot-token-validated","admin-channel-created","admin-form-selectable","admin-chat-discovered","admin-relations-assigned","admin-delivery-targets-assigned-chat","admin-deletion-safety","admin-no-page-errors","admin-no-console-errors"]'
...
Environment:
  ...
  CF7TG_E6_RESULTS_DIR             Summary, logs, traces, screenshots, and Playwright report directory.
  CF7TG_E6_WP_VERSION              WordPress core version, default current support row.
  CF7TG_E6_WP_CLI_IMAGE            WP-CLI image, default current support row.
  CF7TG_E6_WP_WEB_IMAGE            Apache WordPress image. Defaults to versioned image if available.
  CF7TG_E6_WEB_PORT                Host port. Defaults to a random high port.
USAGE
}
...
resolve_web_image() {
	...
	if [ -n "${CF7TG_E6_WP_WEB_IMAGE:-}" ]; then
		WP_WEB_IMAGE="${CF7TG_E6_WP_WEB_IMAGE}"
...
setup_site() {
	...
	run_logged "core-install" wp_run core install --path=/var/www/html --url="${BASE_URL}" --title="CF7TG E6 Form Delivery Smoke" --admin_user=admin --admin_password=admin-password --admin_email=admin@example.test || exit 1
...
run_browser_smoke() {
	...
	(
		...
		CF7TG_E6_BASE_URL="${BASE_URL}" \
		CF7TG_E6_RESULTS_DIR="${RESULTS_DIR}" \
		CF7TG_E6_RESULT_JSON="${BROWSER_RESULT_JSON}" \
		CF7TG_E6_PLAYWRIGHT_REPORT_JSON="${PLAYWRIGHT_REPORT_JSON}" \
